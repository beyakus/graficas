...
export class GraphBarStackedSharedComponent implements OnInit, OnChanges, OnDestroy {
	...
	@Input() colors = ['#00D0CE', '#885ACB', '#FF776C', '#6CA3E4'];
	@Input() keys = ['data1', 'data2', 'data3', 'data4'];
	@Input() descriptionKeys = ['Data 1', 'Data 2', 'Data 3', 'Data 4'];
	@Input() data = [
		{
			date: '07/01/2019',
			data1: 0,
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (
			(data && !data.firstChange) ||
			(paginate && !paginate.firstChange) ||
			(filterApply && !filterApply.firstChange)
		) {
			this.setWindowSize();
			this.getPaddingLabel();
			this.generateDummyData();
	...
	putLabels() {
		if (this.showLabelX) {
			...
			this.svg
				...
				.attr('transform', 'translate(' + this.width + ' ,' + (this.height + this.margin.top + 30) + ')');
		}
	}
	...
	drawGraph() {
		...
		this.x = d3.scaleBand().rangeRound([0, this.width]).paddingInner(this.paddingInner).paddingOuter(0.5);
		...
		this.svg
			...
			.attr('id', 'gradient-bar-stacked-' + this.id)
			.attr('x1', 0)
			.attr('y1', 0)
			.attr('x2', 0)
			.attr('y2', -this.y(0))
		...
		this.xAxis = d3
			.axisBottom(this.x)
			.tickFormat(d3.timeFormat(this.formatAxisX))
			.tickPadding(this.tickPaddingXResponsive);
		this.yAxis = d3
			.axisLeft(this.y)
			.ticks(stepY0)
			.tickValues(d3.range(0, +maxY0 + stepY0, stepY0))
			.tickPadding(50);
		...
		this.y.domain([-(+maxY0 * 0.02), maxY0 + stepY0]);
		this.z.domain(this.keys);

		this.xAxis.tickSize(-this.height, 0, 0).tickSizeOuter(0);
		this.yAxis.tickSizeInner(-this.width).tickSizeOuter(0);
	...
	createTooltip() {
		...
		this.tip = d3Tip()
			...
			.html((d: any) => {
				let html = '<div class="header-tooltip">' + this.titleTooltip + '</div>' + '<div class="body-tooltip">';
	...
	getPpalFormatY() {
		const aux = this.ns
			.transform(d3.max(this.dataReady, (series) => series.total))
			.split(' ')
			.pop();
		this.suffixY = aux;
	}
	...
	prepareData() {
		...
		if (this.isMobile || this.paginateGraph) {
			if (this.currentPage === 1) {
				...
				this.dataReady = this.data.slice(
